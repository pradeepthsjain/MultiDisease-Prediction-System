...
import random  # Import random module for hospital selection
...
if selected == 'Disease Prediction': 
    ...
    if st.button('Predict'): 
        ...
        recommended_hospital = random.choice(hospitals)  # Pick a random hospital
        st.write(f'🏥 {recommended_hospital}')
...
if selected == 'Lung Cancer Prediction':
    ...
    if st.button("Predict Lung Cancer"):
        ...
        if cancer_prediction[0] == 'YES':
            cancer_result = "The model predicts that there is a risk of Lung Cancer. Please visit Adichunchanagiri hospital and get it treated."
            image = Image.open('positive.jpg')
            st.image(image, caption='')
        else:
